      ...
      <div style={{ maxWidth:1200, margin:'0 auto', display:'grid', gridTemplateColumns:`repeat(${cols},1fr)` }}>
        {items.map((item,i) => (
          <motion.div key={i} initial={{ opacity:0, y:30 }} animate={inView?{opacity:1,y:0}:{}} transition={{ duration:0.5, delay:i*0.1 }}
            style={{ padding:'32px 28px', textAlign:'center', borderRight:(i+1)%cols!==0?'1px solid rgba(16,10,98,0.08)':'none', borderBottom: cols<4 && i<items.length-cols ? '1px solid rgba(16,10,98,0.08)' : 'none', position:'relative', overflow:'hidden' }}>
