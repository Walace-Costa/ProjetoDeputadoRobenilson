  ...
  return (
    <a href={m.url} target="_blank" rel="noopener noreferrer"
      ...
      <motion.div whileHover={{ y:-6, borderColor:m.tagColor, boxShadow:`0 12px 40px ${m.tagColor}30` }}
        style={{ background:'rgba(255,255,255,0.06)', border:'1px solid rgba(255,255,255,0.1)', borderRadius:14, overflow:'hidden', boxShadow:'0 2px 10px rgba(0,0,0,0.15)', transition:'border-color 0.25s, box-shadow 0.25s', height:'100%', cursor:'pointer', position:'relative' }}>
        ...
        <div style={{ padding:'14px 16px 18px' }}>
          ...
          <p style={{ color:'#2E2E2E', fontSize:11, lineHeight:1.6, marginBottom:12 }}>{m.resumo}</p>
...
function InfiniteRow({ items, toLeft = false, speed = 40 }) {
  ...
  return (
    <div style={{ overflow:'hidden', position:'relative' }}>
      <div style={{ position:'absolute', left:0, top:0, bottom:0, width:edgeW, background:'linear-gradient(90deg,#eef6f0,transparent)', zIndex:2, pointerEvents:'none' }} />
      <div style={{ position:'absolute', right:0, top:0, bottom:0, width:edgeW, background:'linear-gradient(-90deg,#eef6f0,transparent)', zIndex:2, pointerEvents:'none' }} />
      <motion.div
  ...
  return (
    <section id="mencoes" ref={ref} style={{
      ...
      <motion.div initial={{ opacity:0 }} animate={inView?{opacity:1}:{}} transition={{ delay:0.5 }}
        ...
        <div style={{ display:'inline-flex', flexWrap:'wrap', justifyContent:'center', alignItems:'center', gap:12, background:'#fdfbf3', border:'1px solid rgba(23,105,57,0.2)', borderRadius:12, padding:'12px 28px', boxShadow:'0 2px 10px rgba(16,10,98,0.05)' }}>
