...
export default function Contato() {
  ...
  return (
    <section id="contato" ref={ref} style={{ background: 'var(--cream)', padding: '60px clamp(1rem,4vw,3rem)', position: 'relative', overflow: 'hidden', boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.5)' }}>
                  ...
                  {[{ id: 'nome', label: 'Seu nome', type: 'text', ph: 'João da Silva' }].map(f => (
                    <div key={f.id}>
                      ...
                      <input type={f.type} placeholder={f.ph} required value={form[f.id]} onChange={e => setForm({ ...form, [f.id]: e.target.value })}
                        ...
                        style={{ width: '100%', padding: '12px 14px', border: '1.5px solid rgba(16,10,98,0.4)', borderRadius: 8, fontSize: 14, background: '#fff', color: 'var(--navy)', outline: 'none', fontFamily: 'var(--sans)' }} />
                    </div>
                  ))}
                  <div>
