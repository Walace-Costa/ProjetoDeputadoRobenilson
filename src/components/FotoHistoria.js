  ...
  return (
    <div style={{ position:'relative', width:'100%', height:'100%', minHeight: isMobile ? 280 : 340, display:'flex', alignItems:'center', justifyContent:'center' }}>
      ...
      <motion.button whileHover={{ scale:1.08 }} whileTap={{ scale:0.95 }} onClick={() => setShowAvatar(v=>!v)}
        style={{ position:'absolute', bottom:20, right:20, zIndex:10, background:showAvatar?'rgba(238,243,234,0.88)':`${accent}cc`, border:`1px solid ${accent}55`, borderRadius:10, padding:'9px 15px', color:showAvatar?'var(--navy)':'#fff', fontSize:12, fontWeight:600, cursor:'pointer', display:'flex', alignItems:'center', gap:7, backdropFilter:'blur(8px)' }}>
